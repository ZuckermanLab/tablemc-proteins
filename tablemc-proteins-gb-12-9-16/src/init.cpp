...
#endif
{
    ...
    char buffer[255],word[255],tablefmt[255],covtablefmt[255],fragfmt[255],structf3name[255],structfmt[255];
    char * seq;
    char * pch;
    ...
    old_per_frag_born_radii=(double *) checkalloc(top->nfrag,sizeof(double));
    ...
    new_per_frag_born_radii=(double *) checkalloc(top->nfrag,sizeof(double));

    if (do_mc) {
        //Read start/restart and initial structure.
        fgets(buffer,sizeof(buffer),f);
        sscanf(buffer,"%s %s %s %s\n",word,structfname,structfmt,struct2fname);
        initcoords=NULL;
        ...
        if (start) {
            ...
            if (strlen(struct2fname)>0) {
                ...
                top->write_pdb_file(struct2fname,structfmt,oldcoords,ffield);
            }
            nprevstep=0;
        } else if (restart) {
            initcoords=NULL;
    ...
    //GB info
    fgets(buffer,sizeof(buffer),f);
    top->read_gb_params(buffer,&gb_params,cutoff);
    top->print_gb_info(&gb_params);
...
void simulation::do_energies(char * type, char * fname,  char * enfname)
{
    ...
    if (strcasecmp(type,"rest")==0) {  //Calculate energies from restart file. Needed for replica exchange.
        ...
        total_energy(oldcenter,oldorient,oldcoords,old_per_frag_born_radii,energies,&etot);
    ...
    while (!feof(input)) {
        ...
        if (strcasecmp(type,"pdb")==0) {
            ...
            top->read_frame_quat(input,&istep,oldcenter,oldorient);
