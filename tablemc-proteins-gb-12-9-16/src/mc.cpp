...
double simulation::interaction_energy(int ifrag, int jfrag, double * center, double * orient,double * coords)
{
    ...
    if ((tables_lambda<1.0) || enwrite || (entable<=INVALID_ENERGY)) {
        ...
        //if (ifrag<jfrag) {
        enexact=top->exact_interaction_energy(ffield,pbc,halfboxsize,boxsize,gb_params.epsp,false,ifrag,jfrag,coords);
        enexactcount++;
    }
    /*if (enwrite) {
...
void simulation::moved_energy(bool * moved, bool * movedatoms, double * center, double * orient, double * coords, double * per_frag_born_radii, double * energies, double * etot)
{
    double energy;
    int i,j,jfrag,ifrag;
    ...
    ffield->moved_non_tabulated_energy(gb_params.epsp,false,top->natom,top->atoms,movedatoms,non_tab_list.size(),&non_tab_list[0],coords,energies);
    energies[EN_GB_POLAR]=top->dipolar_gb_energy(&gb_params,per_frag_born_radii,center,orient);
...
void simulation::total_energy(double * center, double * orient, double * coords, double * per_frag_born_radii, double * energies, double * etot)
{
    int i,j,itype,jtype;
    ...
    ffield->non_tabulated_energy(gb_params.epsp,false,top->natom,top->atoms,non_tab_list.size(),&non_tab_list[0],coords,energies);
    energies[EN_GB_POLAR]=top->dipolar_gb_energy(&gb_params,per_frag_born_radii,center,orient);
...
void simulation::fakeloop(void)
{
	...
	//total_energy(newcenter,neworient,newcoords,fresh_energies,&fresh_energy);
        ...
        if ((istep%nsave)==0) {
            ...
            top->write_frame_quat(quatoutput,istep,newcenter,neworient);
        }
    }
    fclose(xyzoutput);
    fclose(quatoutput);
...
{
    const char * hdrfmt = "%15s %15s %15s %15s %15s %15s %15s %15s %15s %15s %15s %15s %15s\n";
    const char * enfmt = "%15s %15ld %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f\n";
    if (hdr) {
        fprintf(output,hdrfmt,title,"Step","Total","Interaction","Cov. intxn","Bonds","Angles","Dihedrals","Impropers","Non-tab VDW","Non-tab Elec","GB Polar","SASA");
    }
    fprintf(output,enfmt,title,istep,etot,energies[EN_INTERACTION],energies[EN_COVALENT_TABLE],energies[EN_BOND],energies[EN_ANGLE],energies[EN_DIHEDRAL],energies[EN_IMPROPER],
        energies[EN_VDW_EXACT],energies[EN_ELEC_EXACT],energies[EN_GB_POLAR],energies[EN_SASA]);
    fflush(output);
}
...
void simulation::mcloop(void)
{
    ...
    starttime=clock();
    top->calculate_born_radii(&gb_params,oldcenter,old_per_frag_born_radii);
    total_energy(oldcenter,oldorient,oldcoords,old_per_frag_born_radii,fresh_energies,&fresh_energy);
    ...
    for (istep=1; istep<=nmcstep; istep++) {
         ...
         natt[movetype]++;
         moved_energy(moved,movedatoms,oldcenter,oldorient,oldcoords,old_per_frag_born_radii,oldenergies,&eold);
         if (use_nb_list) {
#ifdef TIMERS
         ...
         top->calculate_born_radii(&gb_params,newcenter,new_per_frag_born_radii);
         moved_energy(moved,movedatoms,newcenter,neworient,newcoords,new_per_frag_born_radii,newenergies,&enew);
         de=enew-eold;
         ...
         if (r<p) {
             ...
             nacc[movetype]++;
             for (i=0; i<top->nfrag; i++) {
                if (moved[i]) top->copy_frag(i,newcenter,neworient,newcoords,oldcenter,oldorient,oldcoords);
                //all the born radii could have changed
                old_per_frag_born_radii[i]=new_per_frag_born_radii[i];
             }
             //if (de<=-0.5*DUMMY_ENERGY) cum_energy=total_energy(); //This guards against numerical errors related to "declashing."
         } else {
             //REJECTED
             for (i=0; i<top->nfrag; i++) {
                 if (moved[i]) top->copy_frag(i,oldcenter,oldorient,oldcoords,newcenter,neworient,newcoords);
                 new_per_frag_born_radii[i]=old_per_frag_born_radii[i];
             }
         ...
         if ((istep%nprint)==0) {
             ...
             top->calculate_born_radii(&gb_params,newcenter,new_per_frag_born_radii);
             total_energy(newcenter,neworient,newcoords,new_per_frag_born_radii,fresh_energies,&fresh_energy);
