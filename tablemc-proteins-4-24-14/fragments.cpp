...
int topology::fragtypebyname(const char * name)
{
    int itype;
...
int topology::fragtypebyfile(const char * fname)
{
    int itype;
...
topology::topology(const char * commandfile, forcefield * ffield, const char * fragfmt)
{
    FILE * f;
...
void topology::read_residue_definition(FILE * f, residuedef * def)
{
    ...
    while (!feof(f)) {
        ...
        if (token==NULL) continue; //blank line
            ...
            def->nbond++;
        } else if (strncmp("FRAG",token,4)==0) {
            //Read fragment to residue defintiion.
            token=strtok(NULL,delim);
            itype=fragtypebyname(token);
            if (itype<0) {
...
void topology::print_detailed_info(void)
{
    ...
    //Atom info
    printf("index, res. number, atom name, type, class, fragment, frag. atom, list of bonded atoms\n");
    for (iatom=0; iatom<natom; iatom++) {
        printf("%d, %d, %s, %d, %d, %d, %d, ",iatom,atoms[iatom].resNum,atoms[iatom].name,atoms[iatom].type,atoms[iatom].classx,atoms[iatom].fragment,atoms[iatom].fragatom);
	...
	for(k=0;k<=2;k++){
	  if(k!=j){
        ...
        //In the CHARMM 19 parameter set, the FIRST atom is the central atom.
          if((iclass==ffield->impropParams[i].atomClass[0] && jclass==ffield->impropParams[i].atomClass[1] && mclass==ffield->impropParams[i].atomClass[2] && kclass==ffield->impropParams[i].atomClass[3])||
	     (iclass==ffield->impropParams[i].atomClass[0] && jclass==ffield->impropParams[i].atomClass[2] && mclass==ffield->impropParams[i].atomClass[1] && kclass==ffield->impropParams[i].atomClass[3])){
		  //if(jclass==ffield->impropParams[i].atomClass[0] && kclass==ffield->impropParams[i].atomClass[1] && iclass==ffield->impropParams[i].atomClass[2] && mclass==ffield->impropParams[i].atomClass[3]){
		    ...
		    //printf("iatom: %d, jatom: %d, katom: %d, matom: %d, i: %d\n",iatom,jatom,katom,matom,i);
		    break;
		  }
		}
	      }
