...
void table::read_table_header_info(const char * fname)
{
    ...
    hdr.invalidpoints=0;
    //hdr.totalpoints=(long long int) hdr.ntrans * (long long int) hdr.norient;
    fscanf(f,"%lg %d %lg %lg %lg %d %lg\n",&hdr.eps,&rdie,&hdr.beta,&hdr.sph_alpha,&hdr.euler_alpha,&hdr.n_invalid_dev,&hdr.en_invalid_margin); //, &hdr.n_invalid_dev, &hdr.en_clash,&hdr.en_invalid_margin);
    ...
    fgets(hdr.forcefield,sizeof(hdr.forcefield),f);
...
double table::get_pair_energy(forcefield * ffield, fragmenttype * frag1, fragmenttype * frag2, double * coords1, double * coords2, double r, double sphtheta, double sphphi, double phi, double theta, double psi)
{
    double x[3],q[4];
                        sph_to_cart(r,sphtheta,sphphi,&x[0]);
    ...
    return ffield->exact_interaction_energy(FALSE,0.0,0.0,hdr.eps,(hdr.options & OP_DIST_DEP_DIELECTRIC)!=0,frag1->natom,frag1->types,coords1,frag2->natom,frag2->types,coords2);
}

void table::fill_table(forcefield * ffield, fragmenttype * frag1, fragmenttype * frag2)
{
        ...
        for (isphtheta=0; isphtheta<hdr.nsphtheta; isphtheta++)
            for (isphphi=0; isphphi<hdr.nsphphi; isphphi++) {
                for (ir=0; ir<hdr.nr; ir++) {
                    ...
                    sphphi=isphphi*hdr.dsph;
                    en[0]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi,theta,psi);
                    if (hdr.en_invalid_margin>0) {
                        en[1]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r*sqrt(hdr.rfactor),sphtheta,sphphi,phi,theta,psi);
                        en[2]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r/sqrt(hdr.rfactor),sphtheta,sphphi,phi,theta,psi);
                        en[3]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta+0.5*hdr.dsph,sphphi,phi,theta,psi);
                        en[4]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta-0.5*hdr.dsph,sphphi,phi,theta,psi);
                        en[5]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi+0.5*hdr.dsph,phi,theta,psi);
                        en[6]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi-0.5*hdr.dsph,phi,theta,psi);
                        en[7]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi+0.5*hdr.deuler,theta,psi);
                        en[8]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi-0.5*hdr.deuler,theta,psi);
                        en[9]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi,theta+0.5*hdr.deuler,psi);
                        en[10]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi,theta-0.5*hdr.deuler,psi);
                        en[11]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi,theta,psi+0.5*hdr.deuler);
                        en[12]=get_pair_energy(ffield,frag1,frag2,coords1,coords2,r,sphtheta,sphphi,phi,theta,psi-0.5*hdr.deuler);
                        endev=0.0;
                        ndev=0;
                        for (i=1; i<=12; i++) {
...
void table::generate_table(const char * control_file)
{
    long int ir,isphtheta,isphphi,iphi,itheta,ipsi;
    ...
    fill_table(ffield,frag1,frag2);
    //Only compute free energy if no invalid points.
    if (hdr.invalidpoints==0) printf("Association free energy at temperature %.2f K: %.6f kcal/mol\n",1/(KBOLTZ*hdr.beta),assoc_free_energy(hdr.beta));
    if (hdr.sph_alpha>0) {
