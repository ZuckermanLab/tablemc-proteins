...
void simulation::moved_energy(bool * moved, bool * movedatoms, double * center, double * orient, double * coords, double * energies, double * etot)
{
    double energy;
    int i,j,jfrag,ifrag;
    ...
    ffield->moved_non_tabulated_energy(eps,rdie,top->natom,top->atoms,movedatoms,non_tab_list.size(),&non_tab_list[0],coords,energies);
...
void simulation::total_energy(double * center, double * orient, double * coords, double * energies, double * etot)
{
    int i,j,itype,jtype;
    ...
    ffield->non_tabulated_energy(eps,rdie,top->natom,top->atoms,non_tab_list.size(),&non_tab_list[0],coords,energies);
#ifdef TIMERS
    switch_timer(TIMER_INT_OTHER);
#endif
...
#endif
#ifdef EXCHANGE
         if ((istep%exchfreq)==0) exchange(istep/exchfreq,&cum_energy,newcenter,neworient,newcoords);
#endif
