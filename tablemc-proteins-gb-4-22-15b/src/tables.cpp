...
table::table(const char * fmt, const char * fragfmt, const char * fragname1, const char * fragname2, topology * top)
{
    ...
    if (strcasecmp(fragfname1,hdr.ref_frag_fname)==0) {
        reffragtype=top->frag_type_by_name(fragname1);
    } else if (strcasecmp(fragfname2,hdr.ref_frag_fname)==0) {
        reffragtype=top->frag_type_by_name(fragname2);
    }
    if (strcasecmp(fragfname1,hdr.other_frag_fname)==0) {
        otherfragtype=top->frag_type_by_name(fragname1);
    } else if (strcasecmp(fragfname2,hdr.other_frag_fname)==0) {
        otherfragtype=top->frag_type_by_name(fragname2);
    }
...
void table::alloc_read_table(FILE * f, const char * fname)
{
    ...
    filesize=hdr.headersize+hdr.totalpoints*sizeof(energy_t);
    ...
    energy = (energy_t *) malloc(hdr.totalpoints*sizeof(energy_t)); //generate
    if (energy == NULL) {
    ...
    fread(energy,sizeof(energy_t),hdr.totalpoints,f);
#endif
#ifdef NO_CRC
