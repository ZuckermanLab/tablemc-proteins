...
void simulation::exchange(int icycle, double * cum_energies, double * cum_energy, double * center, double * orient, double * coords)
{
    int rep_swap,ifrag,partner,i; //rep_swap and rep_swap+1 will exchange
    double * exchcenter;
    double * exchorient;
    double * exchcoords;
    double * exch_per_atom_born_radii;
    double * exch_per_fragment_born_radii;
    double * exch_dg_self; 
    double * exch_sasa;
    ...
    exch_per_atom_born_radii=NULL;
    exch_per_fragment_born_radii=NULL;
    exch_dg_self=NULL;
    exch_sasa=NULL;
    if ((gb_mode==GB_MODE_PER_ATOM) || (gb_mode==GB_MODE_PER_ATOM_CONVERTED)) {
        exch_per_atom_born_radii=ALLOCATE(top->natom,double);
        exch_dg_self=ALLOCATE(top->natom,double);
        exch_sasa=ALLOCATE(top->natom,double);
    }
    if ((gb_mode==GB_MODE_PER_FRAGMENT) || (gb_mode==GB_MODE_PER_ATOM_CONVERTED)) {
        exch_per_fragment_born_radii=ALLOCATE(top->natom,double);
    }
    if (gb_mode==GB_MODE_PER_FRAGMENT) {
        exch_dg_self=ALLOCATE(top->nfrag,double);
        exch_sasa=ALLOCATE(top->nfrag,double);        
    }
    dummy_moved=(bool *) checkalloc(top->nfrag,sizeof(bool));
    for (ifrag=0; ifrag<top->nfrag; ifrag++) dummy_moved[ifrag]=true;
    ...
    //fflush(stdout);
    MPI_Sendrecv(center,3*top->nfrag,MPI_DOUBLE,partner,NMSG*icycle+2,exchcenter,3*top->nfrag,MPI_DOUBLE,partner,NMSG*icycle+2,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    //printf("node %d exchanged centers with node %d\n",mynod,partner);
    //fflush(stdout);
    MPI_Sendrecv(orient,4*top->nfrag,MPI_DOUBLE,partner,NMSG*icycle+3,exchorient,4*top->nfrag,MPI_DOUBLE,partner,NMSG*icycle+3,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    ...
    for (ifrag=0; ifrag<top->nfrag; ifrag++) top->update_coords(ifrag,exchcenter,exchorient,exchcoords);
         if (use_nb_list) {
             ...
             new_nb_list=frag_nblist->check_nb_list(pbc,halfboxsize,boxsize,cutoff,top->nfrag,dummy_moved,exchcenter);       
            if (new_nb_list) {
    ...
    calculate_born_radii(exchcenter,exchcoords,exch_per_atom_born_radii,exch_per_fragment_born_radii,exch_dg_self,exch_sasa);
    total_energy(exch_per_atom_born_radii,exch_per_fragment_born_radii,exch_dg_self,exch_sasa,exchcenter,exchorient,exchcoords,energies,&eji);
    //exchange these
    ...
    if (accept) {
        ...
        *cum_energy=eji;
        ...
        //it's not necessary to do the "new" ones, will be recalculated on the next mc step
        if ((gb_mode==GB_MODE_PER_ATOM) || (gb_mode==GB_MODE_PER_ATOM_CONVERTED)) for (i=0; i<top->natom; i++) {
            old_per_atom_born_radii[i]=exch_per_atom_born_radii[i];
            old_dg_self[i]=exch_dg_self[i];
            old_sasa[i]=exch_sasa[i];
        }
        if ((gb_mode==GB_MODE_PER_FRAGMENT) || (gb_mode==GB_MODE_PER_ATOM_CONVERTED)) for (i=0; i<top->nfrag; i++) {
            old_per_fragment_born_radii[i]=exch_per_fragment_born_radii[i];
        }
        if (gb_mode==GB_MODE_PER_FRAGMENT) for (i=0; i<top->nfrag; i++) {
         ...
         if (use_nb_list) {
             ...
             new_nb_list=frag_nblist->check_nb_list(pbc,halfboxsize,boxsize,cutoff,top->nfrag,dummy_moved,newcenter);       
            if (new_nb_list) {
                //printf("New nonbond list at step %ld\n",istep);
                //if (pbc) recenter();
                frag_nblist->create_nonbond_list(pbc,halfboxsize,boxsize,top->nfrag,newcenter);       
    ...
    free(exchorient);
    free(exchcoords);
    if (exch_per_atom_born_radii!=NULL) free(exch_per_atom_born_radii);
    if (exch_per_fragment_born_radii!=NULL) free(exch_per_fragment_born_radii);
    if (exch_dg_self!=NULL) free(exch_dg_self);
    if (exch_sasa!=NULL) free(exch_sasa);
}
