...
//Run simulations only in parallel mode (as of right now)
    ...
    if (strcasecmp(argv[1],"generate")==0) {
        ...
        sim->do_energies(argv[3],argv[4],argv[5],argv[6]);
        delete sim;
    } /*else if (strcasecmp(argv[1],"born-test")==0) {
        sim=new simulation("run",argv[2]); //need to include reading/fitting a sturcture.
        sim->born_test(argv[3],atoi(argv[4]),argv[5]);
        delete sim;
