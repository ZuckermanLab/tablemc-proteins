...
void covalent_table::fill_table(topology * top, forcefield * ffield,fragmenttype * fragtype1,fragmenttype * fragtype2, int nb_atom_list_size, atom_nb_entry * nb_atom_list)
{
    ...
    bool * movedatoms;
    gb_param_info fake_gb_params;
    fake_gb_params.epsp=hdr.eps;
...
#endif
                //vacuum only for now.
                ffield->moved_non_tabulated_energy(top->natom,top->atoms,movedatoms,nb_atom_list_size,nb_atom_list,
                    GB_MODE_NONE,fake_gb_params,coords,NULL,energies);
                etot=0.0;
