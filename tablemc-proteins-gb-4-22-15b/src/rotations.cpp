...
void rmsd_fit(int natom, double * weight, double * coords1, double * coords2, double * center, double * orient, double * rmsd)
{
    ...
    *rmsd=sqrt(fabs(aux-2*ev));
    ...
    //This is the net displacement AFTER ROTATING THE ORIGINAL COORDINATES ABOUT THE ORIGIN
    //that must be applied to superimpose the centers of mass.
    //(Rotate first about the origin, then translate.  Do not bring the center of mass to origin first.
    rotate_vector_by_quat(orient,c1center,c1center2);
    //this formula below was buggy in previous codes, but did not show up b/c the COM of a fragment reference geometry
