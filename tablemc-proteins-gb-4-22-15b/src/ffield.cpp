...
//superseded by nonbonded_energy_gb, pass a1a2=0 for vacuum
/*void forcefield::nonbond_energy(int type1,  int type2, bool is14, double r2, double * evdw, double * eelec)
{
    int class1,class2;
    double q1,q2,r6;
    r6=r2*r2*r2;
...
double forcefield::exact_interaction_energy(int pbc, double halfboxsize, double boxsize, double eps, int natom1, int * types1, double * coords1, int natom2, int * types2, double * coords2)
{
    int i,j,class1,class2;
    double r2,eelec, evdw,eelec1,evdw1,egb1;
    eelec=0.0;
    evdw=0.0;
    for (i=0; i<natom1; i++) {
        for (j=0; j<natom2; j++) {
            r2=pbc_distance2(pbc,halfboxsize,boxsize,&coords1[3*i],&coords2[3*j]);
            nonbond_energy_gb(types1[i],types2[j],false,r2,0,&evdw1,&eelec1,&egb1);
            eelec+=eelec1;
            evdw+=evdw1;
        }
    }
...
#ifdef DEBUG
    printf("exact_int_energy_gb_per_frag: VDW ELEC GB %.5f %.5f %.5f\n",evdw,eelec*COUL_CONST/eps,0.0);
#endif
    return eelec*COUL_CONST/eps+evdw;
    //return eelec*COUL_CONST/eps;
}
...
//for use with per-atom born radii
//gb_params is passed by reference here and in exact_interaction_energy_gb_per_atom to avoid copying.  
double forcefield::exact_interaction_energy_gb_per_atom(int pbc, double halfboxsize, double boxsize, gb_param_info * gb_params,
    int natom1, int * types1, double * coords1, double * alpha1, int natom2, int * types2, double * coords2, double * alpha2)
{
    int i,j,class1,class2;
    double r2,eelec, evdw,egb,eelec1,evdw1,egb1,a1a2;
    eelec=0.0;
    evdw=0.0;
    egb=0.0;
    for (i=0; i<natom1; i++) {
        for (j=0; j<natom2; j++) {
            r2=pbc_distance2(pbc,halfboxsize,boxsize,&coords1[3*i],&coords2[3*j]);
            a1a2=alpha1[i]*alpha2[j];
            //printf("exact_int_energy_gb_per_atom1: %d %d %d %d %.5f %.5f\n",i,j,types1[i],types2[j],r2,a1a2);
            //fflush(stdout);
            nonbond_energy_gb(types1[i],types2[j],false,r2,a1a2,&evdw1,&eelec1,&egb1);
            //printf("exact_int_energy_gb_per_atom2: %d %d %.5f %.5f %.5f\n",i,j,evdw1,eelec1*COUL_CONST/gb_params->epsp,egb1*gb_params->tau);
            //fflush(stdout);
            eelec+=eelec1;
            evdw+=evdw1;
            egb+=egb1; //has the minus sign
        }
    }
#ifdef DEBUG
    printf("exact_int_energy_gb_per_atom: VDW ELEC GB %.5f %.5f %.5f\n",evdw,eelec*COUL_CONST/gb_params->epsp,egb*gb_params->tau);
#endif
    return egb*gb_params->tau+eelec*COUL_CONST/gb_params->epsp+evdw;
    //return eelec*COUL_CONST/gb_params->epsp;
    //return evdw;
}

//for use with per-fragment radii
double forcefield::exact_interaction_energy_gb_per_frag(int pbc, double halfboxsize, double boxsize, gb_param_info * gb_params,
    int natom1, int * types1, double * coords1, int natom2, int * types2, double * coords2, double a1a2)
{
    int i,j,class1,class2;
    double r2,eelec, evdw,egb,eelec1,evdw1,egb1;
    eelec=0.0;
    evdw=0.0;
    egb=0.0;
    for (i=0; i<natom1; i++) {
        for (j=0; j<natom2; j++) {
            r2=pbc_distance2(pbc,halfboxsize,boxsize,&coords1[3*i],&coords2[3*j]);
            //printf("exact_int_energy_gb_per_frag1: %d %d %d %d %.5f %.5f\n",i,j,types1[i],types2[j],r2,a1a2);
            //fflush(stdout);
            //We deal with the case a1a2==0 (both fragments are uncharged) in topology::exact_interaction_energy, calling forcefield::exact_interaction_energy instead
            //this is more efficient (not having to test a1a2>0 on every trip through the loop) and we also get floating point exceptions if we try to call nonbond_energy_gb with a1a2=0 
            //if (a1a2>0) {
            nonbond_energy_gb(types1[i],types2[j],false,r2,a1a2,&evdw1,&eelec1,&egb1);
            //printf("exact_int_energy_gb_per_frag2: %d %d %.5f %.5f %.5f\n",i,j,evdw1,eelec1*COUL_CONST/gb_params->epsp,egb1*gb_params->tau);
            //fflush(stdout);
            //} else {
            //    nonbond_energy(types1[i],types2[j],false,r2,&evdw1,&eelec1);
            //}
            eelec+=eelec1;
            evdw+=evdw1;
            egb+=egb1;
...
void forcefield::moved_non_tabulated_energy(int numOfAtoms, ATOMS * atoms, bool * movedatoms, int non_tab_list_size, atom_nb_entry * non_tab_list, int gb_mode, gb_param_info gb_params,
    double * coords, double * born_radii, double * energies)
{
  int i,j,k;
  int ij;
  int ires;
  ...
  double a1a2,egb,egbtot;
  bool is14;
  //all energy terms should have been zeroed out in moved_energy and total_energy
 ...
 //Calculate all interaction terms that need to be calculated exactly.
  ...
  for (i=0; i<non_tab_list_size; i++) {
      ...
      //we can't use this rule anymore, because the born radii could have changed
      ...
      //we assume they're close enough together to be in the same periodic box
      //r2=pbc_distance2(false,0,0,&coords[3*iatom],&coords[3*jatom]);
      switch(gb_mode) {
          case GB_MODE_NONE: a1a2=0; break; //egb will be zero, see condition in nonbond_energy_gb
          case GB_MODE_PER_ATOM:
            a1a2=born_radii[iatom]*born_radii[jatom];break;
        case GB_MODE_PER_ATOM_CONVERTED:
        case GB_MODE_PER_FRAGMENT:
            a1a2=born_radii[atoms[iatom].fragment]*born_radii[atoms[jatom].fragment]; break;
      }

    //this wastes some effort in that we calculate vdw and electrostatic interactions for 1,2 and 1,3 atoms
            //even though we don't need them
        nonbond_energy_gb(atoms[iatom].type,atoms[jatom].type,is14,r2,a1a2,&evdw,&eelec,&egb);
        //if we are using per atom born radii, the self term does not include screening involving atoms in the same fragment, so must add it always.
        //if using per fragment born radii, the self delta G does include this screening so only add it if not part of same fragment.
        //We do count the vdW and electrostatic terms if not a 1,2 or 1,3 relationship though.
        egbtot+=egb;
        if (!non_tab_list[i].is12_or_13) {
      ...
      printf("nonbond: %d %d\n",iatom,jatom);
#endif
  energies[EN_ELEC_EXACT]=energies[EN_ELEC_EXACT]*COUL_CONST/gb_params.epsp;
  energies[EN_GB_EXACT]+=egbtot*gb_params.tau; //already includes deltaG_k from finish_born_radii
#ifdef TIMERS
...
void forcefield::non_tabulated_energy(int numOfAtoms, ATOMS * atoms, int non_tab_list_size, atom_nb_entry * non_tab_list, int gb_mode, gb_param_info gb_params,
    double * coords, double * born_radii, double * energies)
{
  int i,j,k;
  int ij;
...
#ifdef DEBUG
      printf("gb_non_tab: %d %d %d %d %.10f\n",iatom,jatom,atoms[iatom].fragment,atoms[jatom].fragment,egb*gb_params.tau);
#endif
#ifdef DEBUG_NON_TABULATED
  ...
  energies[EN_ELEC_EXACT]=energies[EN_ELEC_EXACT]*COUL_CONST/gb_params.epsp;
  energies[EN_GB_EXACT]=egbtot*gb_params.tau; 
}
