...
#define MAX_NUM_OF_ATOM_TYPES       500
#define COUL_CONST                  332.0716//conversion from electron^2/Ang to kcal/mole
#define COUL_CONST_INV              3.0115599e-3
#define BONDED14_SCALE              0.4
...
#define EN_GB_SELF        8
#define EN_GB_EXACT       9
#define EN_SASA           10
#define EN_TERMS          11 //adjust this if more terms are added!
...
class forcefield {
    ...
    void nonbond_energy(int type1, int type2, bool is14, double r2, double * evdw, double * eelec);
...
public:
    ...
    VDWPARAMS vdwParams[MAX_NUM_OF_ATOM_CLASSES]; //topology::write_pqr_file needs this as well as atomTypeLookup and chargeParams
    ...
    double exact_interaction_energy(int pbc, double halfboxsize, double boxsize, double eps, int natom1, int * types1, double * coords1, int natom2, int * types2, double * coords2);
    double exact_interaction_energy_gb_per_atom(int pbc, double halfboxsize, double boxsize, gb_param_info * gb_params,
        int natom1, int * types1, double * coords1, double * alpha1, int natom2, int * types2, double * coords2, double * alpha2);
    double exact_interaction_energy_gb_per_frag(int pbc, double halfboxsize, double boxsize, gb_param_info * gb_params,
        int natom1, int * types1, double * coords1, int natom2, int * types2, double * coords2, double a1a2);
    void moved_non_tabulated_energy(int numOfAtoms, ATOMS * atoms, bool * movedatoms, int non_tab_list_size, atom_nb_entry * non_tab_list, int gb_mode, gb_param_info gb_params,
        double * coords, double * born_radii, double * energies);
    void non_tabulated_energy(int numOfAtoms, ATOMS * atoms, int non_tab_list_size, atom_nb_entry * non_tab_list, int gb_mode, gb_param_info gb_params,
        double * coords, double * born_radii, double * energies);
