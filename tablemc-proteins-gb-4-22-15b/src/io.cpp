...
void topology::write_pdb_file(char * fname, double * coords)
{
	...
	if ((iatom>0) && (atoms[iatom].fragment!=atoms[iatom-1].fragment)) {
        ...
        fprintf(f,pdbfmt,iatom+1,atoms[iatom].name,atoms[iatom].resName,chain,ires+1,
            coords[3*iatom],coords[3*iatom+1],coords[3*iatom+2],1.0,((double) atoms[iatom].fragment));
    }
    fprintf(f,"END\n");
    fclose(f);
}
...
//This subroutine writes in PQR format, suitable for APBS, but sets up only fragment ichargedfrag with charges from force field.
//if ichargedfrag<0 all atoms are charged
void topology::write_pqr_file(char * fname, double * coords, int ichargedfrag, forcefield * ffield)
{
    FILE * f;
    const char * pqrfmt = "ATOM  %5d %4.4s %3.3s %c%4d    %8.3f%8.3f%8.3f %.4f %.4f # %d\n";
    //const char * pqrfmt = "ATOM %5d %4.4s %3.3s %c %4d %.10f %.10f %.10f %.4f %.4f # %d\n";
    int iatom,ires,iseg,iclass;
    char chain;
	...
	iclass=atoms[iatom].classx;
        if ((ichargedfrag<0) ||(atoms[iatom].fragment==ichargedfrag)) {
	...
	radius=ffield->vdwParams[iclass].sigma;
        //indices are zero-based, add 1 to each
        fprintf(f,pqrfmt,iatom+1,atoms[iatom].name,atoms[iatom].resName,chain,ires+1,
...
{
    int ifrag,i,natomx,nfragx,k,itype;
    double x[3],q[4];
    ...
    //for (i=0;i<nfrags;i++){
        ...
        ifrag=ifrag-1;
        itype=top->frag_type_by_name(name);
        if (itype!=top->frags[ifrag].type) {
...
void simulation::write_restart(long int istep, char * fname)
{
    ...
    for (ifrag=0; ifrag<top->nfrag; ifrag++) fprintf(output,"%d %s %.20f %.20f %.20f %.20f %.20f %.20f %.20f\n",
        ifrag+1,top->fragtypes[top->frags[ifrag].type]->fragname,newcenter[3*ifrag],newcenter[3*ifrag+1],newcenter[3*ifrag+2],
        neworient[4*ifrag],neworient[4*ifrag+1],neworient[4*ifrag+2],neworient[4*ifrag+3]);
    fclose(output);
}
...
{
    int i;
    //this subroutine is called infrequently enough that allocations are permissible
    float * xwrite = ALLOCATE(top->natom,float);
    float * ywrite = ALLOCATE(top->natom,float);
    float * zwrite = ALLOCATE(top->natom,float);
...
{                
    ...
    for (ifragx=0; ifragx<top->nfrag; ifragx++){                 
        ...
        ifrag=ifrag-1;
        if ((ifrag>=top->nfrag) || (strncasecmp(fragname,top->fragtypes[top->frags[ifrag].type]->fragname,MAX_FRAGMENT_NAME)!=0)) {
...
{
    int ifrag;
    for (ifrag=0; ifrag<top->nfrag; ifrag++) fprintf(output,"%ld %d %s %.10f %.10f %.10f %.10f %.10f %.10f %.10f\n",
        istep,ifrag+1,top->fragtypes[top->frags[ifrag].type]->fragname,center[3*ifrag],center[3*ifrag+1],center[3*ifrag+2],orient[4*ifrag],orient[4*ifrag+1],orient[4*ifrag+2],orient[4*ifrag+3]);
    fflush(output);
}
